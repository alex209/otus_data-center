...
### Настроить корректную работу межподового взаимодействия
...
В ходе выполнения данной работы была спроектирована и реализована мультиподовая сеть ЦОД на базе современных технологий EVPN и VXLAN с безадресной IPv6-фабрикой (BGP unnumbered). Проект охватывал два независимых пода (POD1 и POD2), каждый из которых построен по архитектуре Clos с двумя Spine, четырьмя Leaf и двумя Border Leaf. Всего в работе задействовано 24 виртуальных клиента (VPC), подключённых к Leaf через механизм мультихоминга (ESI-LAG), что обеспечивает отказоустойчивость и балансировку нагрузки.

**Разработана масштабируемая архитектура мультиподового ЦОД**
Сеть объединяет две независимые фабрики в единое логическое пространство, при этом каждая фабрика сохраняет свою автономность (разные номера AS, независимые протоколы). Это позволяет легко расширять инфраструктуру добавлением новых подов без изменения существующих.

**Реализован отказоустойчивый underlay с безадресной IPv6 маршрутизацией**
Применение технологии IPv6 unnumbered позволило полностью отказаться от назначения IP-адресов на P2P-линках, что упростило конфигурацию, сократило адресное пространство и обеспечило единообразие настроек всех L3-интерфейсов. BGP-сессии устанавливаются через IPv6 link-local адреса с использованием конструкции neighbor interface, а ECMP гарантирует балансировку трафика.

**Обеспечена мультитенантность и изоляция клиентских сегментов**
Внедрены VRF (RED и BLUE) с соответствующими L2VNI и L3VNI. Клиенты разных VRF полностью изолированы на L3-уровне, при этом внутри одного VRF обеспечена как L2-коммутация, так и L3-маршрутизация между VLAN. Anycast gateway на всех Leaf обеспечивает оптимальную маршрутизацию без «трафика-футбола».

**Реализован мультихоминг (EVPN Ethernet Segment) для серверов**
Серверы подключены к двум Leaf через Port-Channel с использованием ESI-LAG. Это позволяет активно использовать оба канала, автоматически переключать трафик при отказе одного из Leaf или линков, а также синхронизировать состояние Ethernet Segment через EVPN Type-1 маршруты.

**Организовано контролируемое межподовое взаимодействие**
На пограничных маршрутизаторах (Border Leaf) применён подход Option A (VRF-to-VRF) с перезаписью Route Target. Это обеспечивает связность между клиентами разных подов, сохраняя независимость каждой фабрики и позволяя гибко управлять распространяемыми маршрутами.

**Подтверждена работоспособность и отказоустойчивость решения**
Проведено тестирование связности внутри подов и между подами для разных VRF, проверена работа мультихоминга при отключении линков и целых Leaf, выполнены проверки отказоустойчивости Border Leaf. Все сценарии показали корректное поведение сети.

### Заключение
В результате работы спроектирована и реализована современная мультиподовая сеть ЦОД, отвечающая требованиям масштабируемости, отказоустойчивости и мультитенантности.Полученные результаты могут служить основой для дальнейших исследований и внедрения в реальных проектах.
